...
const Experience = () => {
  return (
    <motion.section
      ...
      viewport={{ once: true }}
      className="relative bg-white py-12 px-4 sm:px-6 lg:px-8"
    >
          ...
          <div className=" gap-6 grid md:grid-cols-3">
            {experiences.map((exp, index) => (
              <motion.div
                ...
                viewport={{ once: true }}
                className=" bg-[#F5F6F6]  rounded-[20px] p-6 "
              >
                <div className="flex items-start space-x-4">
                  <div
                    className="p-3 h-16 w-16 flex flex-col justify-center 
                  items-center  bg-white from-blue-500 to-purple-500 rounded-xl"
                  >
                    <Image
                      src={exp.companyLogo}
                      width={56}
                ...
                <div className="flex flex-col  mt-4 space-y-1   text-sm mb-3">
                  ...
                  {exp.achievements.map((achievement, i) => (
                    <div key={i} className="flex items-start    text-sm">
                      <div className="  h-full self-stretch">
                        <div className="w-1.5 h-1.5 mt-2 bg-blue-600 rounded-full mr-3"></div>
                      </div>

                      <div className="flex items-start  ">{achievement}</div>
                    </div>
                  ))}
                </div>
              </motion.div>
